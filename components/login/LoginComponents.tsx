...
export const EmailInput = (State: ChangeValueProps) => {
    ...
    return (
        <Animated.View className=' rounded-xl flex flex-row ' style={[{ backgroundColor: '#DED4E0' }, animatedView]}>
            <TextInput autoCapitalize='none' value={State.value} onChangeText={(txt) => { State.setError(false); State.setValue(txt) }} className={State.error ? 'p-4 py-6 rounded-xl border border-red-500 flex-1' : ' p-4 py-6 rounded-xl border border-secondary flex-1'} placeholder='Email' placeholderTextColor={'#7a7c99'} />
        </Animated.View>
    )
}
...
export const PasswordInput = (State: ChangeValueProps) => {
    ...
    return (
        <Animated.View className={State.error ? ' rounded-xl border border-red-500 flex flex-row' : 'rounded-xl border border-secondary flex flex-row'} style={[{ backgroundColor: '#DED4E0' }, animatedView]}>
            <TextInput autoCapitalize='none' maxLength={32} value={State.value} onChangeText={(txt) => { State.setError(false);State.setValue(txt) }} className=' p-4 py-6 rounded-xl flex-1' placeholder='Password' secureTextEntry={visible} placeholderTextColor={'#7a7c99'} />
